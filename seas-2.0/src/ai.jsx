...
const ChatPanel = ({ open, onClose, screen="overview", onRunAgent, dataset, runningAgent, agentStep }) => {
  ...
  return (
    <aside className="fade-up" style={{
      ...
      {/* header */}
      <div className="ai-border" style={{padding:"12px 14px", borderRadius:14, borderBottomLeftRadius:0, borderBottomRightRadius:0, display:"flex", alignItems:"center", gap:10, background:"linear-gradient(180deg,var(--surface),var(--amber-grad))"}}>
        <div style={{width:28, height:28, borderRadius:8, background:"var(--ink-solid)", color:"var(--ink-solid-fg)", display:"grid", placeItems:"center"}}>
      ...
      {mcpOpen && (
        <div className="fade-up" style={{padding:"10px 14px", borderBottom:"1px solid var(--line)", background:"var(--surface-2)"}}>
          ...
          <div style={{display:"flex", flexDirection:"column", gap:6}}>
            {mcpCatalog.map(s=>(
              <div key={s.id} style={{display:"flex", alignItems:"center", gap:10, padding:"8px 10px", background:"var(--surface)", border:"1px solid var(--line)", borderRadius:8}}>
      ...
      <div style={{padding:"10px 12px", borderTop:"1px solid var(--line)", background:"var(--surface-2)"}}>
        <div style={{display:"flex", alignItems:"flex-end", gap:8, background:"var(--surface)", border:"1px solid var(--line)", borderRadius:10, padding:"6px 8px"}}>
          <textarea
            value={input}
...
const ChatMessage = ({ m, agentPlan }) => {
  if(m.role==="user"){
    return (
      <div style={{display:"flex", justifyContent:"flex-end", marginBottom:10}}>
        <div style={{maxWidth:"85%", background:"var(--ink-solid)", color:"#f2f1ec", padding:"9px 12px", borderRadius:"12px 12px 2px 12px", fontSize:13, lineHeight:1.45}}>
          {m.text}
        </div>
      </div>
    );
  ...
  if(m.role==="tool"){
    return (
      <div style={{marginBottom:10, background:"var(--surface)", border:"1px solid var(--line)", borderRadius:10, padding:"8px 10px"}}>
  ...
  return (
    <div style={{marginBottom:12, display:"flex", gap:8}}>
      <div style={{width:22, height:22, borderRadius:7, background:"var(--ink-solid)", color:"var(--ink-solid-fg)", display:"grid", placeItems:"center", flexShrink:0}}>
...
const AgentRun = ({ step=3 })=>{
  return (
    <div className="ai-border" style={{borderRadius:12, padding:10, background:"var(--surface)", marginTop:6}}>
